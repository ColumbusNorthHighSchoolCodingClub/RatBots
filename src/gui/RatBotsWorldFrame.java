...
//        }));
        ...
        //begin recording-related buttons
        
        JMenu recMenu = makeMenu("menu.recording");        
        
        ButtonGroup group = new ButtonGroup();
        JMenuItem item = new JRadioButtonMenuItem();
        configureMenuItem(item,"menu.recording.recorderOn", new ActionListener(){
            public void actionPerformed(ActionEvent e){
                ((RatBotWorld)world).setIsRecording(true);
            }
        });
        group.add(item);
        recMenu.add(item);
        
        item = new JRadioButtonMenuItem("Deactivate grid recorder", true);
        configureMenuItem(item,"menu.recording.recorderOff", new ActionListener(){
            public void actionPerformed(ActionEvent e){
                System.out.println("recorder deactivated");
                ((RatBotWorld)world).setIsRecording(false);
            }
        });
        group.add(item);
        recMenu.add(item);
        recMenu.addSeparator();
        recMenu.add(makeMenuItem("menu.recording.undo", new ActionListener(){
            public void actionPerformed(ActionEvent e){
                System.out.println("Move undone             RatBotsWorldFrame");
                ((RatBotWorld)world).undo();
            }
        }));
        
        recMenu.add(makeMenuItem("menu.recording.jump", new ActionListener()
        {
            public void actionPerformed(ActionEvent e)
            {
                JOptionPane bob = new JOptionPane();
                
                String input = bob.showInputDialog("Input round number:");
                try{
                    int round = Integer.parseInt(input);   
                    ((RatBotWorld)world).jumpToMove(round);   
        ...
        recMenu.add(makeMenuItem("menu.recording.load", new ActionListener(){
            public void actionPerformed(ActionEvent e){
                JFileChooser chooser = new JFileChooser();
                FileNameExtensionFilter filter = new FileNameExtensionFilter("txt files", "txt");
                chooser.setFileFilter(filter);
                int returnVal = chooser.showDialog(mbar, "Load");
                if(returnVal==chooser.APPROVE_OPTION){
                    File f = chooser.getSelectedFile();
                    ((RatBotWorld)world).setMatchFile(f);
                    ((RatBotWorld)world).beginReplay(1);
                }
